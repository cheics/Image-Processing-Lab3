...
\subsection{Using the Weiner filter for recovering noisy images}

The Weiner filter is able to compensate for the noise present in a degraded image, and reduces the strength of the inverse filter across the image to prevent overaccomadating noise.
...
\centering
	...
	\caption{Weiner filtering with various NSR. As NSR $\rightarrow$0, Weiner filtering becomes normal inverse filtering.}
\end{figure}
...
Wiener filtering is able to accomadate noise when attempting to solve the noise problem inherrent in deconvolution. The NSR, if unknown, must be carefully chosen in order to to get a good result.
